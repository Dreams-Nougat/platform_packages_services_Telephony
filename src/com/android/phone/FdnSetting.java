...
public class FdnSetting extends PreferenceActivity implements
    ...
    private void updateEnableFDN(long subId) {
        mPhone = PhoneProxyManager.getPhoneProxyUsingSub(subId);
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (getIntent().hasExtra(PhoneConstants.SIM_ID_KEY)) {
            mSubId = getIntent().getLongExtra(
                    PhoneConstants.SIM_ID_KEY, SubscriptionController.SIM_NOT_INSERTED);
    ...
    public boolean onPreferenceClick(Preference preference) {
        if (preference == mButtonFDNList) {
            startActivity(preference.getIntent().putExtra(PhoneConstants.SUB_ID_KEY, mSubId));
        }
        return true;
    }
