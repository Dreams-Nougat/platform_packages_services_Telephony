...
public class OutgoingCallBroadcaster extends Activity
    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public boolean doReceive(Context context, Intent intent) {
            ...
            final Phone phone = PhoneUtils.getPhoneUsingSub(
    ...
    private void processIntent(Intent intent) {
        ...
        if (callNow) {
            ...
            long subId = intent.getLongExtra(PhoneConstants.SUBSCRIPTION_KEY, SubPickHandler.INVALID_SUB_ID);
    ...
    public void onSubPickComplete(int completeKey, long subId, Intent intent) {
        ...
        if (completeKey == SubPickHandler.SUB_PICK_COMPLETE_KEY_SUB) {
            ...
            startSipCallOptionHandler(getApplicationContext(), intent, uri, number);
            startDelayedFinish();
        } else if (completeKey == SubPickHandler.SUB_PICK_COMPLETE_KEY_CANCEL) {
            finish();
        }
    }
    ...
    private void doSubPick(Context context, Intent intent, String number, boolean withSipItem) {
        ...
        if (PhoneUtils.getSimCount() == 1 && !withSipItem) {
            ...
            if (PhoneUtils.getActivatedSubInfoCount(context) == 0) {
                ...
                intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subInfoRecord.mSubId);
                sendNewCallBroadcaster(intent, number, false);
                return;
            } else {
        ...
        List<SubPickItem> subPickItems = SubPickHandler.getSubPickItemList(context, withSipItem, true);
    ...
    public static boolean isInCellNetwork() {
        ...
        for (int i = 0; i < simCount; i++) {
            final Phone phone = PhoneUtils.getPhoneUsingSim(i);
