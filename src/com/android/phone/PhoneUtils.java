...
public class PhoneUtils {
    ...
    */
    ...
    public static Phone pickPhoneBasedOnNumber(CallManager cm,
        ...
        if (isValidSubId(PhoneGlobals.getInstance(), subId)) {
            return PhoneProxyManager.getPhoneProxyUsingSub(subId);
        }
    ...
    static final void registerIccStatus(Handler handler, int event) {
        ...
        for (Phone phone : phones) {
            sim = phone.getIccCard();
            if (sim != null) {
                if (VDBG) Log.v(LOG_TAG, "register for ICC status, sim " + phone.getSimId());
    ...
    static Phone pickBestPhoneForEmergencyCall() {
        ...
        for (Phone phone : phones) {
            if (ServiceState.STATE_IN_SERVICE == phone.getServiceState().getState()) {
                ...
                log("pickBestPhoneForEmergencyCall, radio on & in service, simId:"
                        + phone.getSimId());
                return phone;
            } else if (ServiceState.STATE_POWER_OFF != phone.getServiceState().getState()) {
                // the slot is radio on
                log("pickBestPhoneForEmergencyCall, radio on, simId:" + phone.getSimId());
                if (selectPhone == null) {
                    selectPhone = phone;
                }
            }
    ...
    public static List<SubInfoRecord> getActivatedSubInfoList(Context context) {
        List<SubInfoRecord> subInfoLists = SubscriptionController.getActivatedSubInfoList(context);
     ...
     * @return
     */
    public static boolean isValidSubId(Context context, long subId) {
        boolean isValid = false;
        List<SubInfoRecord> activeSubInfoList = SubscriptionController.getActivatedSubInfoList(context);
    ...
    public static void checkAndCopySubExtras(Context context, Intent src, Intent dst) {
        ...
        long subId = src.getLongExtra(PhoneConstants.SUB_ID_KEY, SubPickHandler.INVALID_SUB_ID);
        if (isValidSubId(context, subId)) {
            dst.putExtra(PhoneConstants.SUB_ID_KEY, subId);
        }
    }
    ...
    public static List<SubPickItem> getSubPickItemList(Context context,boolean withSipItem, boolean withUnInsertSim) {
        ...
        List<SubInfoRecord> activeSubInfoLists = getActivatedSubInfoList(context);
        ...
        for (int i = 0; i < isSimInsertArray.length; i++) {
            isSimInsertArray[i] = false;
        }
