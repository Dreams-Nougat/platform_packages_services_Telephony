...
public class NotificationMgr {
    ...
    /* package */ void updateCfi(int simId) {
        ...
        if (mCfiStatusMap.get(simId)) {
            ...
            SubInfoRecord simInfo = SubscriptionManager.getSubInfoUsingSimId(PhoneGlobals.getInstance(), simId).get(0);
    ...
    void updateNetworkSelection(Phone phone) {
        if (TelephonyCapabilities.supportsNetworkSelection(phone)) {
            ...
            int simId = SubscriptionManager.getSubInfoUsingSubId(mContext, phone.getSubscription()).mSimId;
            if (!PhoneUtils.isValidSim(simId)) {
                if (DBG) log("updateNetworkSelection(), invalid simId, do nothing!");
                return;
            }
            String networkSelection =
                    sp.getString(PhoneBase.NETWORK_SELECTION_NAME_KEY + simId, "");
            if (TextUtils.isEmpty(networkSelection)) {
                networkSelection =
                        sp.getString(PhoneBase.NETWORK_SELECTION_KEY + simId, "");
            }
