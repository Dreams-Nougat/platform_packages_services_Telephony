...
public class NotificationMgr {
    ...
    private void initCfiStatusMap() {
        ...
        for (Phone phone : phones) {
            mCfiStatusMap.put(phone.getSimId(), false);
        }
    }
    ...
    /* package */ void updateMwi(boolean visible, int simId) {
        ...
        if (visible) {
            ...
            if (phone == null) {
                if (DBG) log("updateMwi(): phone is null, do nothing");
                return;
            }
            ...
            intent.putExtra(PhoneConstants.SUB_ID_KEY, simId);
    ...
    /* package */ void updateCfi(boolean visible, int simId) {
        ...
        Phone[] phones = PhoneProxyManager.getDefault().getPhoneProxys();
        for (int index = phones.length - 1; index >= 0; index--) {
            int sim = phones[index].getSimId();
            updateCfi(sim);
        }
    }
     ...
     */
    private void showNetworkSelection(String operator, long subId) {
        ...
        int notificationId = SELECTED_OPERATOR_FAIL_NOTIFICATION + (int)subId * NOTIFICATION_ID_OFFSET;
     ...
     */
    private void cancelNetworkSelection(long subId) {
    ...
    void updateNetworkSelection(Phone phone) {
        if (TelephonyCapabilities.supportsNetworkSelection(phone)) {
            ...
            long subId = SubscriptionController.getSubId(phone.getSimId())[0];
